...
function handle_item_insert(&$item_r, $HTTP_VARS, &$errors) {
	if ((is_user_granted_permission ( PERM_ITEM_OWNER ) && $item_r ['owner_id'] == get_opendb_session_var ( 'user_id' )) || is_user_granted_permission ( PERM_ITEM_ADMIN )) {
		...
		if (is_valid_item_type_structure ( $item_r ['s_item_type'] )) {
					...
					if (! $is_exists_title && ! $is_exists_owner_title) {
						if ($HTTP_VARS ['trial_run'] != 'true') {
							...
							if ($new_item_id !== FALSE) {
								...
								$db_error = db_error ();
								$errors = array (
										'error' => get_opendb_lang_var ( 'item_not_added' ),
										'detail' => $db_error );
								return FALSE;
							}
						} else {	//if($HTTP_VARS['trial_run'] != 'true')
							return TRUE;
						}
					} else if ($HTTP_VARS ['confirmed'] != 'false') {	// if explicitly false, then we are aborting insert.
						if ($is_exists_owner_title) {
							$errors = array (
									'error' => get_opendb_lang_var ( 'title_same_type_and_owner_exists', array (
											'title' => $item_r ['title'],
							...
							$errors = array (
									'error' => get_opendb_lang_var ( 'title_same_type_exists', array (
											'title' => $item_r ['title'],
											's_item_type' => $item_r ['s_item_type'] ) ),
									'detail' => '' );
			...
			$errors = array (
					'error' => get_opendb_lang_var ( 'invalid_item_type_structure', 's_item_type', $item_r ['s_item_type'] ),
					'detail' => '' );
	...
	} else {	// non-admin user attempting to insert item for someone else.
		$errors = array (
				'error' => get_opendb_lang_var ( 'operation_not_available' ) );
	...
	if ($item_r ['owner_id'] == get_opendb_session_var ( 'user_id' ) || is_user_granted_permission ( PERM_ITEM_ADMIN )) {
		if (validate_item_attributes ( 'update', $item_r ['s_item_type'], $HTTP_VARS, $errors )) {
			...
			if (update_item ( $item_r ['item_id'], $item_r ['title'] )) {
				...
				$db_error = db_error ();
				$errors = array (
						'error' => get_opendb_lang_var ( 'item_not_updated' ),
						'detail' => $db_error );
				return FALSE;
			}
		} else {
...
function handle_item_instance_insert(&$item_r, $status_type_r, $HTTP_VARS, &$errors) {
	...
	if ((is_user_granted_permission ( PERM_ITEM_OWNER ) && $owner_id == get_opendb_session_var ( 'user_id' )) || is_user_granted_permission ( PERM_ITEM_ADMIN )) {
		...
		if (is_newinstance_status_type_valid ( $item_r ['item_id'], $owner_id, $status_type_r, $errors )) {
			...
			if ($HTTP_VARS ['trial_run'] != 'true') {
				...
				if ($new_instance_no !== FALSE) {
					...
					$db_error = db_error ();
					$errors = array (
							'error' => get_opendb_lang_var ( 'item_instance_not_added' ),
							'detail' => $db_error );
					return FALSE;
				}
			} else {
		...
		$errors = array (
				'error' => get_opendb_lang_var ( 'not_authorized_to_page' ) );
		...
		if ($item_r ['owner_id'] == get_opendb_session_var ( 'user_id' ) || is_user_granted_permission ( PERM_ITEM_ADMIN )) {
			...
			if ($item_r ['s_status_type'] == $update_status_type || is_update_status_type_valid ( $item_r ['item_id'], $item_r ['instance_no'], $item_r ['owner_id'], $status_type_r, $update_status_type_r, $errors )) {
					...
					if ($status_type_r ['change_owner_ind'] == 'Y') {
						if (! is_item_borrowed ( $item_r ['item_id'], $item_r ['instance_no'] )) {
							if (! update_item_instance_owner ( $item_r ['item_id'], $item_r ['instance_no'], $item_r ['owner_id'], $HTTP_VARS ['owner_id'] )) {
								$errors = array (
										'error' => get_opendb_lang_var ( 'item_instance_owner_not_changed' ) );
							}
						} else {
							$errors = array (
									'error' => get_opendb_lang_var ( 'item_instance_owner_not_changed' ) );
						...
						$errors = array (
								'error' => get_opendb_lang_var ( 'operation_not_avail_change_owner' ),
								'detail' => '' );
					}
				}
				...
				if (update_item_instance ( $item_r ['item_id'], $item_r ['instance_no'], $update_status_type, $status_comment, $borrow_duration )) {
					...
					$db_error = db_error ();
					$errors = array (
							'error' => get_opendb_lang_var ( 'item_instance_not_updated' ),
							'detail' => $db_error );
					return FALSE;
				}
			} else {
		...
		while ( $item_attribute_type_r = db_fetch_assoc ( $results ) ) {
			...
			if ($order_no !== FALSE) {
				...
				if (is_not_empty_array ( $attribute_val_r )) {
					insert_item_attributes ( $new_item_id, $new_instance_no, 					// instance_no
						$new_item_type, $item_attribute_type_r ['s_attribute_type'], $order_no, $attribute_val_r );
				}
			} //if($order_no!==FALSE)
		}
