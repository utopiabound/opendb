...
function fetch_newitem_status_type_rs() {
	$query = "SELECT sst.s_status_type, sst.s_status_type as value, IFNULL(stlv.value, sst.description) as display, sst.img, sst.default_ind as checked_ind " . "FROM s_status_type sst " . "LEFT JOIN s_table_language_var stlv
...
function fetch_update_status_type_rs($status_type) {
	$query = "SELECT DISTINCT sst.s_status_type, sst.s_status_type as value, IFNULL(stlv.value, sst.description) as display, sst.img, sst.default_ind as checked_ind " . "FROM s_status_type sst " . "LEFT JOIN s_table_language_var stlv
			ON stlv.language = '" . get_opendb_site_language () . "' AND
			stlv.tablename = 's_status_type' AND
			stlv.columnname = 'description' AND
			stlv.key1 = sst.s_status_type " . "WHERE ";
...
function is_update_status_type_valid($item_id, $instance_no, $owner_id, $old_status_type_r, $new_status_type_r, &$errors) {
	...
	if ($new_status_type_r ['closed_ind'] != 'Y') {
		...
		$errors = array (
				'error' => get_opendb_lang_var ( 's_status_type_not_supported', 's_status_type_desc', $new_status_type_r ['description'] ),
				'detail' => '' );
		return FALSE;
	}
}
...
{
			$errors = array (
					'error' => get_opendb_lang_var ( 'operation_not_avail_new_instance' ),
					'detail' => '' );
			return FALSE;
		}
